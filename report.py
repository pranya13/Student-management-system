...
class ReportClass:
    def __init__(self, root):
        ...
        title = Label(self.root, text="Student Report", font=("Tahoma", 20, "bold"), bg="#b3cde0", fg="#009a93").place(x=10, y=15, width=1180, height=35)
        self.var_id = ""
        lbl_search = Label(self.root, text="Search By Roll no", font=("goudy old style", 20, "bold"), bg="white").place(x=300, y=100)
        txt_search = Entry(self.root, textvariable=self.var_search, font=("goudy old style", 20), bg="lightblue").place(x=520, y=100, width = 150)
        btn_search = Button(self.root, text = "Search", font = ("goudy old style", 20, "bold"), bg = "#2196f3", fg="white", cursor = "hand2", command=self.search).place(x=680, y=100, width=100, height=35)
        ...
        lbl_marks = Label(self.root, text="Marks", font=("goudy old style", 20, "bold"), bg="white", bd=2, relief=GROOVE).place(x=600, y=230, width=150, height=50)
        ...
        btn_delete = Button(self.root, text = "Delete", font = ("goudy old style", 15, "bold"), command=self.delete, bg = "red",fg = "white", cursor = "hand2").place(x=500, y=350, width=150, height=35)
            ...
            if self.var_id=="":
                ...
                if row == None:
                    ...
                    if op == True:
                        ...
                        con.commit()
                        messagebox.showinfo("Delete","Successfully Deleted Result", parent = self.root)
                        self.clear()
        except Exception as ex:
            messagebox.showerror("Error", f"Error due to {str(ex)}")
